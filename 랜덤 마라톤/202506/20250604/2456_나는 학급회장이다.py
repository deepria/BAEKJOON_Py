def main():
    ...
    arr = [*map(lambda x: sorted(list(x), reverse=True),
                zip(*[[*map(int, read[i].split())] for i in range(1, int(read[0]) + 1)]))]
    sums = sorted([(sum(a), a.count(3), a.count(2), i + 1) for i, a in enumerate(arr)], reverse=True)
    if sums[0][:3] != sums[1][:3]:
        print(sums[0][3], sums[0][0])
    else:
        print(0, sums[0][0])
