def main():
    ...
    i = 0
    while True:
        l, p, v = map(int, read[i].split())
        if l == p == v == 0:
            break
        vp = v // p
        remainder = v % p
        answer = vp * l + min(remainder, l)
        stdout.write(f'Case {i + 1}: {answer}\n')
        i += 1
...
모든 입력 정수는 int 범위이다. 마지막 줄에는 0이 3개 주어진다.

출력
각 테스트 케이스에 대해서, 
강산이가 캠핑장을 최대 며칠동안 사용할 수 있는지 예제 출력처럼 출력한다.
